  ...
  <div>
    ...
    <div class="table-wrapper">
      <table>
        ...
        <tbody>
          <template v-for="phoneacc in phoneaccs">
            <tr>
              <td>{{$niceNumber(phoneacc.chat_count)}}</td>
              <td>{{$niceField(phoneacc.phone_number)}}</td>
              <td></td>
            </tr>
          </template>
        </tbody>
      </table>
      <p class="no-rows" v-if="phoneaccs.length==0">Non è stato trovato alcun account wildix, modifica i parametri di ricerca.</p>
  ...
  export default {
    ...
    methods: {
      ...
      getInitValues(type=null){
        ...
        axios.get('/api/partners/'+this.partner.id+'/phoneaccs', params).then(response => {
          var obj = response.data;
          if(obj && !obj.error){
            this.phoneaccs = obj.phoneaccs;

            if(type=='filters'){
              this.current_page = 1;
            }

            this.page_number = Math.ceil(obj.phoneaccs_count/this.limit);
