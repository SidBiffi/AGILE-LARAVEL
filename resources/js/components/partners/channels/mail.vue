  ...
  <div>
    ...
    <div class="table-wrapper">
      <table>
        ...
        <tbody>
          <template v-for="mailacc in mailaccs">
            <tr>
              <td>{{$niceNumber(mailacc.chat_count)}}</td>
              <td>{{$niceField(mailacc.email_address)}}</td>
              <td></td>
            </tr>
          </template>
        </tbody>
      </table>
      <p class="no-rows" v-if="mailaccs.length==0">Non è stato trovato alcun account mail, modifica i parametri di ricerca.</p>
  ...
  export default {
    ...
    methods: {
      ...
      getInitValues(type=null){
        ...
        axios.get('/api/partners/'+this.partner.id+'/mailaccs', params).then(response => {
          var obj = response.data;
          if(obj && !obj.error){
            this.mailaccs = obj.mailaccs;

            if(type=='filters'){
              this.current_page = 1;
            }

            this.page_number = Math.ceil(obj.mailaccs_count/this.limit);
