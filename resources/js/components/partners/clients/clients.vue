      ...
      <table>
        ...
        <tbody>
          <template v-for="client in clients">
            <tr>
              <td>{{$niceFullName(client)}}</td>
              <td>1234</td>
              <td>000 000 0000</td>
              <td>nomecognome@mail.com</td>
              <td></td>
            </tr>
          </template>
        </tbody>
      </table>
      <p class="no-rows" v-if="clients.length==0">Non è stato trovato alcun utente, modifica i parametri di ricerca.</p>
  ...
  export default {
    ...
    methods: {
      ...
      getInitValues(type=null){
        ...
        axios.get('/api/partners/'+this.partner.id+'/clients', params).then(response => {
          ...
          if(obj && !obj.error){
            ...
            this.page_number = Math.ceil(obj.clients_count/this.limit);
