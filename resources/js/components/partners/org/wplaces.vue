  ...
  <div>
    ...
    <div class="table-wrapper">
      <table>
        ...
        <tbody>
          <template v-for="wplace in wplaces">
            <tr>
              <td>{{$niceField(wplace.name)}}</td>
              <td>{{$niceNumber(wplace.users_count)}}</td>
              <td>1234</td>
              <td>1234</td>
              <td>1234</td>
              <td>1234</td>
      ...
      <p class="no-rows" v-if="wplaces.length==0">Non è stato trovato alcun utente, modifica i parametri di ricerca.</p>
  ...
  export default {
    ...
    methods: {
      ...
      getInitValues(type=null){
        ...
        axios.get('/api/partners/'+this.partner.id+'/wplaces', params).then(response => {
          var obj = response.data;
          if(obj && !obj.error){
            this.wplaces = obj.wplaces;

            if(type=='filters'){
              this.current_page = 1;
            }

            this.page_number = Math.ceil(obj.wplaces_count/this.limit);
