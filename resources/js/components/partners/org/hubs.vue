<template>
  <div>
    ...
    <div class="table-wrapper">
      <table>
        ...
        <tbody>
          <template v-for="hub in hubs">
            <tr>
              <td>{{$niceField(hub.name)}}</td>
              <td>{{$niceNumber(hub.wplaces_count)}}</td>
              <td>1234</td>
              <td>1234</td>
              <td>1234</td>
              <td>1234</td>
      ...
      <p class="no-rows" v-if="hubs.length==0">Non è stato trovato alcun utente, modifica i parametri di ricerca.</p>
  ...
  export default {
    ...
    methods: {
      ...
      getInitValues(type=null){
        ...
        axios.get('/api/partners/'+this.partner.id+'/hubs', params).then(response => {
          ...
          if(obj && !obj.error){
            ...
            this.page_number = Math.ceil(obj.hubs_count/this.limit);
